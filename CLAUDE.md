...
文字模型：
key `sk-3ODOY96LmDCFgcBY1d1b586c01E448BcAbB5115bD8FbD2Fc`， OPENAI_BASE_URL=http://38.246.250.87:3000/v1
...
对于有多语言source的文字稿，都要下载并能切换，上传的时候也有这个选项。而且优先默认显示的是该主题原始的语言

新功能：侧边栏添加小电视，支持直接接入youtube、bilibili等可以直接嵌入的播放器和播放链接，并且点击文案可以跳转到对应的位置，播放时左侧也同步高亮当前播放的位置。尽可能接入能实现接入的所有平台内容并标注出处。（没有视频的可以纯音频）
