...
async function polishEpisode(db, episodeId) {
  ...
  // Get best source transcript (prefer asr_diarized > asr > manual)
  const source = db.prepare(`
    SELECT content, format, source FROM transcripts
    WHERE episode_id=? AND source IN ('asr_diarized', 'asr', 'manual')
    ORDER BY
      CASE source WHEN 'asr_diarized' THEN 0 WHEN 'asr' THEN 1 ELSE 2 END,
      created_at DESC
    LIMIT 1
  `).get(episodeId);
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    while (retries < MAX_RETRIES) {
      ...
      // Validate output length (strip metadata from input for fair comparison)
      const textOnly = chunk.replace(/\[\d{1,3}:\d{2}\]\s*/g, '').replace(/\[SPEAKER_\d+\]\s*/g, '').length;
      const ratio = polished.length / (textOnly || chunk.length);
      if (ratio < MIN_OUTPUT_RATIO && textOnly > 200) {
