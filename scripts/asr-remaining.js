...
const eps = db.prepare(`
  ...
  WHERE p.id IN (16, 17) AND e.id NOT IN (SELECT episode_id FROM transcripts)
  ORDER BY p.id, e.id
`).all();

console.log(`\n🎤 ASR: ${eps.length} episodes\n`);
...
(async () => {
  ...
  for (let i = 0; i < eps.length; i++) {
    ...
    const tmpFile = `/tmp/asr_r_${ep.id}`;
    try {
      console.log('  DL...');
      if (audioUrl.includes('bilibili')) {
        spawnSync('yt-dlp', ['-x', '--audio-format', 'mp3', '-o', tmpFile + '.%(ext)s', '--no-playlist', '--quiet', audioUrl], { timeout: 600000 });
      } else {
        execSync(`curl -L -s -o "${tmpFile}.m4a" --max-time 600 "${audioUrl}"`, { timeout: 610000 });
      }
      const dlFile = fs.readdirSync('/tmp').filter(f => f.startsWith(`asr_r_${ep.id}`)).map(f => '/tmp/' + f).find(f => fs.statSync(f).size > 10000);
    ...
    finally { fs.readdirSync('/tmp').filter(f => f.startsWith(`asr_r_${ep.id}`)).forEach(f => { try { fs.unlinkSync('/tmp/' + f); } catch(e) {} }); }
  }
